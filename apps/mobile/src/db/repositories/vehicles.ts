...
export async function upsertVehicle(
  db: SQLiteDatabase,
  vehicle: Omit<Vehicle, 'created_at' | 'sync_status' | 'server_id'> &
    Partial<Pick<Vehicle, 'created_at' | 'sync_status' | 'server_id'>>,
): Promise<void> {
  await db.runAsync(
    `INSERT INTO vehicles (id, user_id, make, model, year, vin, sync_status, server_id)
     VALUES ($id, $user_id, $make, $model, $year, $vin, $sync_status, $server_id)
