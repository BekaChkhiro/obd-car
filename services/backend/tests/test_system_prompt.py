...
# ── live-data availability (three states) ─────────────────────────────────────


def test_adapter_unknown_omits_the_live_data_statement():
    """No claim is better than a wrong one when the caller does not know."""
    result = build_system_prompt()
    text = "".join(b["text"] for b in result)
    assert "Live vehicle data:" not in text


def test_real_adapter_says_available():
    result = build_system_prompt(adapter_connected=True)
    text = result[1]["text"]
    assert "Live vehicle data: AVAILABLE" in text
    assert "SIMULATED" not in text


def test_no_adapter_says_unavailable():
    result = build_system_prompt(adapter_connected=False)
    text = result[1]["text"]
    assert "Live vehicle data: UNAVAILABLE" in text


def test_simulated_adapter_is_its_own_state():
    """Demo mode is neither of the other two: tools work, readings are fake."""
    result = build_system_prompt(adapter_connected=True, adapter_simulated=True)
    text = result[1]["text"]
    assert "Live vehicle data: SIMULATED" in text
    assert "Live vehicle data: AVAILABLE" not in text
    assert "Live vehicle data: UNAVAILABLE" not in text


def test_simulated_block_says_tools_work_and_values_are_generated():
    result = build_system_prompt(adapter_connected=True, adapter_simulated=True)
    text = result[1]["text"].lower()
    # Both halves have to be there: drop the first and the assistant refuses to
    # demo anything, drop the second and it passes invented numbers off as the
    # user's car.
    assert "tools do work" in text
    assert "generated by that simulator" in text
    assert "not as something you read off their car" in text


def test_simulated_flag_is_ignored_when_nothing_is_connected():
    """`simulated` only qualifies a live link — it never creates one."""
    result = build_system_prompt(adapter_connected=False, adapter_simulated=True)
    text = result[1]["text"]
    assert "Live vehicle data: UNAVAILABLE" in text
    assert "SIMULATED" not in text


def test_simulated_pid_list_is_not_attributed_to_the_vehicle():
    result = build_system_prompt(
        adapter_connected=True, adapter_simulated=True, supported_pids=["010C"]
    )
    text = result[1]["text"]
    assert "simulator's mode-01 availability bitmap" in text
    assert "vehicle's own mode-01 availability bitmap" not in text
